...
public class Solution1{
    public static void main(String[] args){
        ...
        findSum(arr,n,0,0,list);
        int ans=list.get(0);
        for(int i=1;i<list.size();i++) ans=ans | list.get(i); 
        System.out.print(ans);
        
    }
    public static void findSum(int[] arr,int n,int ind,int sum,List<Integer> list){
        if(ind==n){
            if(!list.contains(sum)) list.add(sum);
            return;
        }
        int num=arr[ind];
        findSum(arr,n,ind+1,sum+num,list);
        findSum(arr,n,ind+1,sum,list);
    }
}