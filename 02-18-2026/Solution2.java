...
class Solution2{
    public static void main(String[] args){
        ...
        for(int i=0;i<arr.length;i++) arr[i]=Integer.parseInt(inp[i]);
        TreeNode root=buildTree(arr);
        int ans =updateTree(root,0);
        printTree(root);
    ...
    public static int updateTree(TreeNode root,int sum){
        if(root==null) return sum;
        sum= updateTree(root.right,sum);
        root.val+=sum;
        sum=root.val;
        sum =updateTree(root.left,sum);
        return sum;
    }
